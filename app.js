...
function ensureGlobalModals(){
  ...
  if(!document.getElementById("intervisteModal")){
    const modal = document.createElement("div");
    modal.id = "intervisteModal";
    modal.className = "player-modal";
    modal.setAttribute("onclick", "closeIntervisteModal(event)");
    modal.innerHTML = `<div class="player-modal-content" id="intervisteModalContent"></div>`;
    document.body.appendChild(modal);
  }
}
...
function openNewsModal(id){
  ...
  const immagine = String(item.immagine || "").trim();
  ...
  content.innerHTML = `
    ${immagine ? `
      <div class="modal-header">
        <img src="${escapeHtml(immagine)}" alt="${escapeHtml(title)}">
      </div>
    ` : ``}
    <div class="modal-body">
      ...
      ${link ? `<div style="margin-top:14px;"><a class="mvp-btn" href="${escapeHtml(link)}" target="_blank" rel="noopener">Apri link</a></div>` : ``}
    </div>
  `;
...
function openIntervisteModal(){
  ...
  content.innerHTML = `
    <div class="modal-body">
      <h2 style="margin:0;">Tutte le interviste</h2>
      <div class="meta" style="margin-top:6px;">Archivio completo</div>
      ...
      <div style="margin-top:14px;">
        ${all.length ? all.map(n => {
          ...
          `;
        }).join("") : `<div class="meta" style="margin-top:10px;">Nessuna intervista pubblicata</div>`}
      </div>
    </div>
  `;

  const modal = document.getElementById("intervisteModal");
  if(modal) modal.style.display = "flex";
}

function closeIntervisteModal(e){
  if(e && e.target && e.target.id === "intervisteModal"){
    const modal = document.getElementById("intervisteModal");
    if(modal) modal.style.display = "none";
  }
}
...
function renderNews(){
  ...
  const intervisteAll = news
    .map((n, idx) => ({...n, _id: n.id ?? idx}))
    .filter(n => (n.categoria || "").toLowerCase() === "intervista")
    .slice()
    .sort((a,b)=> (b.data||"").localeCompare(a.data||""));

  const interviste = intervisteAll.slice(0, 3);
  ...
  const blockInterviste = `
    <div class="card">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
        <h3 style="margin:0;">Interviste</h3>
        ${intervisteAll.length > 3 ? `<button class="subtab" onclick="openIntervisteModal()">Vedi tutte</button>` : ``}
      </div>
